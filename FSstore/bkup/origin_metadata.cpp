...
// ディレクトリファイルの読み書き 
void read_dir_file( vector<string> *dir_entry, char* store_path, char*dir_fname);
...
// メタデータの読み書き
void read_metadata(   f_inf *metadata, char* store_path, char* metadata_fname );
void take_finf(  f_inf * finf,   char * full_path,  char * fname    );
void write_metadata( f_inf *metadata, char* store_path, char* metadata_fname );
...
void make_metadata(char * q_name){
	...
	conv_hash( full_path, metadata_fname );
	...
	// 既存メタデータの読み込み
	read_metadata( &metadata, store_path, metadata_fname);
	// ファイルのメタデータを取得
	take_finf( &metadata, full_path, fname );	
	// メタデータの書き込み 
	write_metadata( &metadata, store_path, metadata_fname );	
}
...
void read_metadata( f_inf *metadata, char* store_path, char* metadata_fname ){
	...
	strcat( metadata_path, metadata_fname );
